...
common.uploadFileToStorage = function (key, filePath) {
  ...
  } else if (storageType === "huaweiObs") {
    return common.uploadFileToHuaweiObs(key, filePath);
  }
...
common.uploadFileToHuaweiObs = function (key, filePath) {
  return new Promise((resolve, reject) => {
    var obsClient = new ObsClient({
      //推荐通过环境变量获取AKSK，这里也可以使用其他外部引入方式传入，如果使用硬编码可能会存在泄露风险。
      //您可以登录访问管理控制台获取访问密钥AK/SK，获取方式请参见https://support.huaweicloud.com/intl/zh-cn/usermanual-ca/ca_01_0003.html
      access_key_id: _.get(config, "huaweiObs.accessKeyId"),
    ...
    obsClient.putObject(params, (err, result) => {
      if (err) {
        console.error("huawei obs putObject Error-->" + err);
        reject(err);
      } else {
